...
Read `refs/cached-prices.md`. Check the `Last updated` date in the header:
...
1. **`refs/cached-prices.md`** (primary) — Cached prices (±5-25% accuracy). Set `pricing_source: "cached"`. Used first because it requires zero API calls and covers most common services.
...
- `refs/cached-prices.md` — Cached AWS + source provider pricing (±5-25%, primary source)
